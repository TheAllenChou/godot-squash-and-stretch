...
func reset(init_value:Vector3, init_velocity:Vector3 = Vector3.ZERO) -> void:
  value = init_value
  velocity = init_velocity

func track_damping_ratio(target:Vector3, angular_frequency:float, damping_ratio:float, delta_time:float) -> Vector3:
...
func track_half_life(target:Vector3, frequency_hz:float, half_life:float, delta_time:float) -> Vector3:
...
func track_exponential(target:Vector3, half_life:float, delta_time:float) -> Vector3:
  if half_life < SquashAndStretchUtil.EPSILON:
    velocity = Vector3.ZERO
    value = target
    return value
